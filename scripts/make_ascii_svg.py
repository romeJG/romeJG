...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "rome-ascii.svg")
...
CONTRAST = 1.25
BRIGHTNESS = 1.0
GAMMA = 1.0           # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
WHITE_FLOOR = 0.96    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">romeJG@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'romeJG@github:~$ whoami <tspan fill="{INK}">Rome Guillermo</tspan></text>')
